          ...
          <form class="" method="post" action="submit.php">
			...
			<div class="form-group"> 
				...
				<select name="id_comune_residenza" class="form-control <?=$messaggio ? "is-invalid" : ""?>" >
					...
					?>
					<option <?=(array_key_exists('id_comune_residenza', $old) && $old['id_comune_residenza'] == $comune->getId()) ? "selected" : "" ?> value="<?=$comune->getId()?>"><?=$comune->getNome()?> (<?=$comune->getProvincia()?>) - <?=$comune->getRegione()?> - <?=$comune->getStato()?></option>
					<?php 
					} 
					?>
				</select>
			...
			<div class="form-group"> 
				...
				<select name="id_comune_domicilio" class="form-control <?=$messaggio ? "is-invalid" : ""?>">
					<option value="">Selezionare il comune di domicilio...</option>
					<?php 
					foreach($elencoComuni as $comune) {
					?>
					<option <?=(array_key_exists('id_comune_domicilio', $old) && $old['id_comune_domicilio'] == $comune->getId()) ? "selected" : "" ?> value="<?=$comune->getId()?>"><?=$comune->getNome()?> (<?=$comune->getProvincia()?>) - <?=$comune->getRegione()?> - <?=$comune->getStato()?></option>
					<?php 
					} 
					?>
				</select>
